...
void pblk_map_invalidate(struct pblk *pblk, struct ppa_addr ppa)
{
	...
	if (pblk->l_mg.nr_lines <= line_id) {
		printk("pblk map invalidate %llu %d\n", ppa.ppa, line_id);
		BUG_ON(1);
	}
				  ...
				  unsigned int nr_secs)
{
	...
	sector_t end_lba = slba + nr_secs;
#endif

//	printk("pblk_invalidate slba:%llu nr_secs:%lu\n", slba, nr_secs);

	spin_lock(&pblk->trans_lock);
//	printk("invalidate_range trans_lock start\n");
	...
	for (lba = slba; lba < slba + nr_secs; lba++) {
...
#else
		int mtype = pblk_get_maptype(pblk, lba);
		...
		if (mtype == BLOCK_MAP)
		{
			...
			int i;
			if (loff != 0) {
				lba = lba + (entry_num - loff) - 1;
				continue;
			}
			if (end_lba < lba + entry_num)
				break;

			for (i = 0; i < entry_num; i++)
			{
				ppa = pblk_trans_map_get(pblk, lba + i);

				if (!pblk_addr_in_cache(ppa) && !pblk_ppa_empty(ppa))
					pblk_map_invalidate(pblk, ppa);
				pblk_ppa_set_empty(&ppa);
				pblk_trans_map_set(pblk, lba + i, ppa);
			}
			lba = lba + (entry_num - loff) - 1;
			continue;
		}
		//printk("trans_map_get: %lu\n", lba);
		ppa = pblk_trans_map_get(pblk, lba);
		//printk("trans_map_get end\n");
		if (pblk_addr_in_cache(ppa)) {
			ppa = pblk_cache_invalidate(pblk, ppa);
		}

		//printk("ppa invalid start\n");
		if (!pblk_addr_in_cache(ppa) && !pblk_ppa_empty(ppa))
			pblk_map_invalidate(pblk, ppa);
		//printk("ppa invalid end\n");
		pblk_ppa_set_empty(&ppa);
...
//	printk("invalidate_range trans_lock end\n");
...
static void pblk_write_kick_all(struct pblk *pblk)
{
...
static void pblk_write_kick(struct pblk *pblk, unsigned int nrb)
{
	wake_up_process(pblk->rb_ctx[nrb].writer_ts);
	mod_timer(&pblk->wtimer, jiffies + msecs_to_jiffies(1000));
}
...
{
	unsigned int secs_avail = pblk_rb_read_count(&pblk->rb_ctx[nrb].rwb);

	if (secs_avail >= pblk->min_write_pgs)
		pblk_write_kick(pblk, nrb);
}
...
static void pblk_flush_writer(struct pblk *pblk, unsigned int nrb)
{
	pblk_rb_flush(&pblk->rb_ctx[nrb].rwb, nrb);
	do {
		if (!pblk_rb_sync_count(&pblk->rb_ctx[nrb].rwb))
			break;
...
struct ppa_addr pblk_get_lba_map(struct pblk *pblk, sector_t lba)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_get_lba_map trans_lock start\n");
	ppa = pblk_trans_map_get(pblk, lba);
//	printk("pblk_get_lba_map trans_lock end\n");
	spin_unlock(&pblk->trans_lock);
...
static struct pblk_line *pblk_line_retry(struct pblk *pblk,
					 struct pblk_line *line, unsigned int nrb)
{
	struct pblk_line_mgmt *l_mg = &pblk->l_mg;
...
#ifdef CONFIG_PBLK_MULTIMAP
	retry_line->pstream = nrb;
#endif

	pblk_line_free(pblk, line);
	l_mg->data_line[nrb] = retry_line;
	spin_unlock(&l_mg->free_lock);
...
static void pblk_set_space_limit(struct pblk *pblk, unsigned int nrb)
{
	struct pblk_rl_per_rb *rb_rl = &pblk->rb_ctx[nrb].rb_rl;

	atomic_set(&rb_rl->rb_space, 0);
}
...
struct pblk_line *pblk_line_get_first_data(struct pblk *pblk, unsigned int nrb)
{
	...
	l_mg->data_line[nrb] = line;
#ifdef CONFIG_PBLK_MULTIMAP
	line->pstream = nrb;
	pblk->max_seq = l_mg->d_seq_nr;
	line->ctime = atomic_long_read(&pblk->g_writes);
#endif

	pblk_line_setup_metadata(line, l_mg, &pblk->lm);

	/* Allocate next line for preparation */
	l_mg->data_next[nrb] = pblk_line_get(pblk);
	if (!l_mg->data_next[nrb]) {
		/* If we cannot get a new line, we need to stop the pipeline.
		 * Only allow as many writes in as we can store safely and then
		 * fail gracefully
		 */
		pblk_set_space_limit(pblk, nrb);

		l_mg->data_next[nrb] = NULL;
	} else {
		l_mg->data_next[nrb]->seq_nr = l_mg->d_seq_nr++;
		l_mg->data_next[nrb]->type = PBLK_LINETYPE_DATA;
#ifdef CONFIG_PBLK_MULTIMAP
		l_mg->data_next[nrb]->ctime = atomic_long_read(&pblk->g_writes) + 4096;
#endif
		is_next = 1;
		#ifdef CONFIG_PBLK_MULTIMAP
		pblk->max_seq = l_mg->d_seq_nr;
	...
	if (pblk_line_erase(pblk, line)) {
		line = pblk_line_retry(pblk, line, nrb);
		if (!line)
			return NULL;
	}
	...
	pblk_rl_free_lines_dec(&pblk->rl, line);
	if (is_next) {
		pblk_rl_free_lines_dec(&pblk->rl, l_mg->data_next[nrb]);
	}
...
static void pblk_stop_writes(struct pblk *pblk, struct pblk_line *line, unsigned int nrb)
{
...
{
	struct pblk_line_mgmt *l_mg = &pblk->l_mg;
	int ret = 0, i, nr_rwb = pblk->nr_rwb;
...
#ifndef CONFIG_PBLK_MULTIMAP
	for (i = 0; i < nr_rwb; i++)
		pblk_flush_writer(pblk, i);
#endif
	pblk_wait_for_meta(pblk);
	
	for (i = 0; i < nr_rwb; i++)
		ret = pblk_recov_pad(pblk, i);
	if (ret) {
...
void pblk_line_replace_data(struct pblk *pblk, unsigned int nrb)
{
	...
	cur = l_mg->data_line[nrb];
	new = l_mg->data_next[nrb];
	if (!new)
		return;
	l_mg->data_line[nrb] = new;
#ifdef CONFIG_PBLK_MULTIMAP
	new->ctime = atomic_long_read(&pblk->g_writes);
#endif
	spin_lock(&l_mg->free_lock);
...
retry_setup:
	if (!pblk_line_init_metadata(pblk, new, cur)) {
		new = pblk_line_retry(pblk, new, nrb);
		if (!new)
			return;
	...
	if (!l_mg->data_next[nrb]) {
		...
		is_next = 1;
		#ifdef CONFIG_PBLK_MULTIMAP
		l_mg->data_next[nrb]->pstream = nrb;
		l_mg->data_next[nrb]->ctime = atomic_long_read(&pblk->g_writes) + 4096;
		pblk->max_seq = l_mg->d_seq_nr;
		#endif
	}
...
void pblk_line_put(struct kref *ref)
{
	...
	int nrb = line->pstream;
	atomic_long_inc(&pblk->nr_erase[nrb]);
...
void pblk_update_map(struct pblk *pblk, sector_t lba, struct ppa_addr ppa)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_update_map: lba:%lu\n", lba);
	l2p_ppa = pblk_trans_map_get(pblk, lba);

#ifdef CONFIG_PBLK_MULTIMAP
	if (pblk_addr_in_cache(ppa)) {
		l2p_ppa = pblk_cache_invalidate(pblk, l2p_ppa);
	}
#endif
	...
	pblk_trans_map_set(pblk, lba, ppa);
//	printk("pblk_update_map: trans_lock end\n");
	spin_unlock(&pblk->trans_lock);
}
...
void pblk_update_map_cache(struct pblk *pblk, sector_t lba, struct ppa_addr ppa)
{
...
#ifdef CONFIG_NVM_DEBUG
	int nrb = pblk_addr_to_nrb(ppa);
	/* Callers must ensure that the ppa points to a cache address */
	BUG_ON(!pblk_addr_in_cache(ppa));
	BUG_ON(pblk_rb_pos_oob(&pblk->rb_ctx[nrb].rwb, pblk_addr_to_cacheline(ppa)));
#endif
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_update_map_cache: lba:%lu\n", lba);
	l2p_ppa = pblk_trans_map_get(pblk, lba);
	...
	if (!pblk_addr_in_cache(l2p_ppa) && !pblk_ppa_empty(l2p_ppa)) {
		...
		pblk->lstream[nrb].invalid_line = line_id;
		pblk_map_invalidate(pblk, l2p_ppa);
	}
	pblk_cache_map_set(pblk, lba, ppa);
//	printk("pblk_update_map_cache: trans_lock end\n");
	spin_unlock(&pblk->trans_lock);
#endif
}
		       ...
		       struct pblk_line *gc_line, struct ppa_addr* rppa)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_update_map_gc: %lu\n", lba);
	l2p_ppa = pblk_trans_map_get(pblk, lba);

	if (pblk_addr_in_cache(l2p_ppa)) {
		l2p_ppa = pblk_get_reserved_old_addr2(pblk, l2p_ppa);		
	}
/*
	if (pblk_addr_reserved(l2p_ppa)) {
		*rppa = l2p_ppa;
		ret = 0;
		goto out;
	}
*/
	...
	if (pblk_get_log_maptype(pblk, lba) == BLOCK_MAP)
	{
		printk("GC Block Map Select l2p:%lu lstream:%d maptype:%d\n", 
						lba, pblk_get_lstream(pblk, lba), 
						pblk_get_maptype(pblk, lba));
		ret = 0;
		goto out;
	}
	...
	pblk_trans_map_set(pblk, lba, ppa);
out:
//	printk("pblk_update_map_gc: trans_lock end\n");
	spin_unlock(&pblk->trans_lock);
		       ...
		       struct pblk_line *gc_line)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_update_map_gc trans_lock start aa\n");
	l2p_ppa = pblk_trans_map_get(pblk, lba);
	...
	pblk_trans_map_set(pblk, lba, ppa);
out:
//	printk("pblk_update_map_gc trans_lock end bb\n");
	spin_unlock(&pblk->trans_lock);
	return ret;
}
#endif
			 ...
			 struct ppa_addr entry_line)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_update_map_dev trans_lock start\n");
...
//			pblk_dev_map_set_ignore(pblk, lba, ppa);
			pblk_map_invalidate(pblk, ppa);
		}
		goto out;
	}
	...
	pblk_trans_map_set(pblk, lba, ppa);
#else
	pblk_dev_map_set(pblk, lba, ppa);
#endif

out:
//	printk("pblk_update_map_dev end: %llu trans_lock\n", lba);
	spin_unlock(&pblk->trans_lock);
			 ...
			 sector_t blba, int nr_secs)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_lookup_l2p_seq start:trans_lock\n");
	for (i = 0; i < nr_secs; i++) {
		ppas[i] = pblk_trans_map_get(pblk, blba + i);
		if (pblk_addr_in_cache(ppas[i]))
			ppas[i] = pblk_get_reserved_old_addr(pblk, ppas[i]);

	}
//	printk("pblk_lookup_l2p_seq end: trans_lock\n");
	spin_unlock(&pblk->trans_lock);
			  ...
			  u64 *lba_list, int nr_secs)
{
	...
	spin_lock(&pblk->trans_lock);
//	printk("pblk_lookup_l2p_rand start: trans_lock\n");
	for (i = 0; i < nr_secs; i++) {
		lba = lba_list[i];
		if (lba == ADDR_EMPTY) {
			...
			if (pblk_addr_in_cache(ppas[i]))
				ppas[i] = pblk_get_reserved_old_addr2(pblk, ppas[i]);
		}
	}
//	printk("pblk_lookup_l2p_rand end: trans_lock\n");
	spin_unlock(&pblk->trans_lock);
}
