...
static int sort_and_cut_data(int *nr_group, unsigned long long *fgroup, unsigned long long *data, int *weight)
{
    int min_pointer, i;
    char *visit_bitmap;
	int n = *nr_group;
	unsigned long long *temp_fgroup, *temp_data;
	int *temp_weight;
	*nr_group = n * (VALID_FGROUP_RATE) / 100;

    visit_bitmap = (char*) vmalloc(sizeof(char) * n);

    temp_fgroup = (unsigned long long*) vmalloc(sizeof(unsigned long long) * n);
    temp_data = (unsigned long long*) vmalloc(sizeof(unsigned long long) * n);
    temp_weight = (int*) vmalloc(sizeof(int) * n);
    ...
    for (i = 0; i < n; i++) {
		...
		temp_fgroup[i] = fgroup[min_index];
		temp_data[i] = data[min_index];
		temp_weight[i] = weight[min_index];
        visit_bitmap[min_index] = 1;
    }

	for (i = 0; i < n; i++) {
		fgroup[i] = temp_fgroup[i];
		data[i] = temp_data[i];
...
static int kmeans_cluster_minmax(struct f2fs_sb_info* sbi, unsigned long long *data, int *weight, int *cluster, int n, int k)
{
	int min_pointer;
	char *visit_bitmap;
	int *sort_index;
	...
	for (i = 0; i < k; i++) {
		weight_minbound[i] = weight_sum[i] * (50 - (STREAM_LIFE_RANGE)/2) / 100; 
...
static int f2fs_update_cluster(struct f2fs_sb_info *sbi)
{
	...
	fgroup = vmalloc(sizeof(unsigned long long) * nr_fgroup);
	data = vmalloc(sizeof(unsigned long long) * nr_fgroup);
	weight = (int*) vmalloc(sizeof(int) * nr_fgroup);
	...
	while (n) {
		...
		lifetime = update_new_ema(sbi, entry);
		if (lifetime == 0) {
	//		entry->cluster = nr_cluster - 1;
//			lifetime = user_data_blocks(sbi) - entry->create_time;
//			entry->cluster = f2fs_lifetime_to_cluster(sbi, lifetime);
	...
	result = (int*) vmalloc(sizeof(int) * nr_fgroup);
...
#ifdef KMEANS_ENABLE
	prev_fgroup = nr_fgroup;
	sort_and_cut_data(&nr_fgroup, fgroup, data, weight);
#ifndef CLUSTER_TIME
	printk("PREV: %d --> AFTER: %d\n", prev_fgroup, nr_fgroup);
#endif
	if (sbi->kmeans_count < 10)
		init_center(nr_fgroup, nr_cluster, data, center, weight);
	else {
		for (i = 0; i < nr_cluster; i++)
			center[i] = sbi->kmeans_center[i];
	}

	//convert_data(nr_fgroup, nr_cluster, data, center, weight);
	kmeans_org(nr_fgroup, nr_cluster, data, center, result);
#elif (defined FIX_HOT)
	init_center(nr_fgroup, nr_cluster, data, center, weight);
	kmeans_org(nr_fgroup, nr_cluster, data, center, result);

//	fixed_center(nr_fgroup, NR_CLUSTER-1, data, center, result);
#else
error
	uniform(nr_fgroup, NR_CLUSTER, data, center, result, weight);
#endif
	...
	cluster_arr = (int*) vmalloc(sizeof(int) * nr_fgroup);
    for (j = 0; j < prev_fgroup; j++) {
        int cluster;
		if (j < nr_fgroup) {
	        int index = result[j];
			if (index < 0)
...
#ifndef CLUSTER_TIME
				printk("[CLUSTER]\t%u\t%llu\t%llu\t%llu\t%llu\t%d\t%s\n", cluster, data[j], fgroup_entry->cold, fgroup_entry->valid, fgroup_entry->count, filetype, fgroup_entry->keyword);
#endif
			}
...
#endif
	for (i = 0; i < nr_cluster; i++) {
		sbi->kmeans_max[i] = max_cluster[i];
		sbi->kmeans_center[i] = center[i];
#ifndef CLUSTER_TIME
		printk("KMENAS: center(%llu) max(%llu) curTime(%llu)\n", center[i], max_cluster[i], cur_seq);
#endif
	}
	kmeans_cluster_minmax(sbi, data, weight, cluster_arr, nr_fgroup, nr_cluster);
	for (i = 0; i < nr_cluster; i++) {
