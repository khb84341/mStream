...
static void truncate_node(struct dnode_of_data *dn)
{
	...
	if (blkaddr != NEW_ADDR) {
	#ifdef F2FS_FGROUP
		if (sbi->node_block[blkaddr] == 0) {
			stat_dec_node_cold(sbi);
		}
		else {
			stat_dec_node_update(sbi);
		}
	#endif

		if (sbi->first_gc_time_block[blkaddr] > 0) {
			trace_printk("[EARLY-GC-i]\t%d\t%u\t%u\t%u\t%llu\t%s\t%u\n",
				sbi->group_before_gc[blkaddr], sbi->create_time_block[blkaddr],
				sbi->first_gc_time_block[blkaddr], sbi->real_gc_time_block[blkaddr], cur_seq, "NODE",dn->nid);
			stat_inc_gc_invalid(sbi, sbi->group_before_gc[blkaddr]);
		} else {
			trace_printk("[INVALID-i]\t%d\t%u\t%u\t%u\t%llu\t%s\t%u\n",
				sbi->group_before_gc[blkaddr], sbi->create_time_block[blkaddr],
				sbi->first_gc_time_block[blkaddr], sbi->real_gc_time_block[blkaddr], cur_seq, "NODE", dn->nid);
