...
#ifdef F2FS_ORG_TEST
#ifdef F2FS_TRACE_ENABLE
			snprintf(fi->i_keyword, 50, "KEY-2:%s", extlist[i]);
#endif
#endif
			break;
		}
...
#ifdef F2FS_ORG_TEST
	if (is_multimedia_file(name, "db")) {
		file_set_hot(inode);
#ifdef F2FS_TRACE_ENABLE
		snprintf(fi->i_keyword, 50, "KEY-0:%s", "db");
#endif
	} else {
#ifdef F2FS_TRACE_ENABLE
		fi->i_nameset = 0;
		snprintf(fi->i_keyword, 50, "KEY-1");
#endif
	}
#endif
}
...
#ifdef F2FS_FGROUP
	if (F2FS_I(inode)->i_filetype == FGROUP_INIT)
		F2FS_I(inode)->i_fgroup = get_fgroup(sbi, inode, dentry);
#endif
