...
//    femu_log("ppa(0x%lx): ch(%lu), lun(%lu), blk(%lu), pg(%lu), pl(%lu),"
 //            "sec(%lu)\n", ppa, ch, lun, blk, pg, pln, sec);
   if (blk == 9999909)
   printf("ppa(0x%lx): ch(%lu), lun(%lu), blk(%lu), pg(%lu), pl(%lu),"
             "sec(%lu)\n", ppa, ch, lun, blk, pg, pln, sec);
                           ...
                           NvmeRequest *req, AddrBucket *bucket, int *nr)
{
    ...
    for (int i = 0; i < nlb; i++) {
        ...
        //    printf("i: %d, ppa: %x, cur_pg_addr:%x ch:%d, lun:%d, pg:%d, page_type:%u\n", i, (unsigned int)ppa, (unsigned int)cur_pg_addr, bucket[secs_idx].ch, bucket[secs_idx].lun, bucket[secs_idx].pg, bucket[secs_idx].page_type);
                               ...
                               NvmeRequest *req)
{
    ...
    if (init_trace == 1) {
        write_count += secs_idx;
	if (write_count % 100000 == 0)
		printf("write_count:%d\n", write_count);
//	if (write_count > 4600000) {	// EXT4
#if 0
	if (write_count > 5300000) {	// F2FS
	...
	//printf("i:%d ch: %d, page_type:%u, lun: %d, lunid: %d\n", i, ch, page_type, lun, lunid);
    ...
    if (opcode == OC12_CMD_WRITE) {
        //printf("size: %d, emulate_time:%lu now:%lu, expire_time:%lu\n", secs_idx, total_time_need_to_emulate, now, req->expire_time);
    }
    return 0;
}
                           ...
                           NvmeRequest *req)
{
	...
	//printf("i:%d lbads:%u ppa:%lx psl[i]:%lx\n", i, lbads, ppa, psl[i]);
    }
...
static void oc12_init_id_ctrl(Oc12Ctrl *ln)
{
    ...
    ln_id->ppaf.pln_offset  = ln_id->ppaf.sect_offset + ln_id->ppaf.sect_len;
    ln_id->ppaf.pln_len     = qemu_fls(cpu_to_le16(ln->params.num_pln) - 1);
    ln_id->ppaf.ch_offset   = ln_id->ppaf.pln_offset + ln_id->ppaf.pln_len;
    ln_id->ppaf.ch_len      = qemu_fls(cpu_to_le16(ln->params.num_ch) - 1);
    ln_id->ppaf.lun_offset  = ln_id->ppaf.ch_offset + ln_id->ppaf.ch_len;
    ln_id->ppaf.lun_len     = qemu_fls(cpu_to_le16(ln->params.num_lun) - 1);
    ln_id->ppaf.pg_offset   = ln_id->ppaf.lun_offset + ln_id->ppaf.lun_len;
    ln_id->ppaf.pg_len      = qemu_fls(cpu_to_le16(ln->params.pgs_per_blk) - 1);
    ln_id->ppaf.blk_offset  = ln_id->ppaf.pg_offset + ln_id->ppaf.pg_len;
    ln_id->ppaf.blk_len     = qemu_fls(cpu_to_le16(ln->id_ctrl.groups[0].num_blk) - 1);
#endif
...
static int oc12_init_more(FemuCtrl *n)
{
    ...
    for (int i = 0; i < n->num_namespaces; i++) {
        ...
        ln->ppaf.blk_mask = ((1 << ppaf->blk_len) - 1)  << ln->ppaf.blk_offset;
#endif
	printf("MASK: sec:%x pln:%x pg:%x blk:%x lun:%x ch:%x\n", (unsigned int)ln->ppaf.sec_mask,
		(unsigned int)ln->ppaf.pln_mask, 
		(unsigned int)ln->ppaf.pg_mask, 
		(unsigned int)ln->ppaf.blk_mask, 
		(unsigned int)ln->ppaf.lun_mask, 
		(unsigned int)ln->ppaf.ch_mask);
	printf("OFFSET: sec:%u pln:%u pg:%u blk:%u lun:%u ch:%u\n",
		(unsigned int)ln->ppaf.sec_offset,
		(unsigned int)ln->ppaf.pln_offset,
		(unsigned int)ln->ppaf.pg_offset,
		(unsigned int)ln->ppaf.blk_offset,
		(unsigned int)ln->ppaf.lun_offset,
		(unsigned int)ln->ppaf.ch_offset);
